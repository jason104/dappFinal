...
async function fetchTransactions(celebrityList) {
    document.write(`Fetching recent Txs of ${celebrityList[0][0]}, ${celebrityList[1][0]}, ${celebrityList[2][0]} and some other celebrities.`);
    //Get current time
        ...
        .then((response) => {
            ...
            for (let j = 0; j < results.length; j++) {
                ...
                if (Number(results[j].value) >= 10 * (10 ** 18)) {
                    document.write('Address ' + results[j].from + ' has sent ' + results[j].value / (10 ** 18) + ' ETH to Address ' + results[j].to + '.\n' + celebrityList[i][1] + ' is ' + celebrityList[i][0] + "'s account.\n");
                }
            }
        })
